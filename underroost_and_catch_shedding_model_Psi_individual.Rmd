...
ct.fit.prob = readRDS("ct.fit.prob.39cutoff.RDS")
...
catch.prev$var1[catch.prev$catch.shed==1] = rnorm(sum(catch.prev$catch.shed==1),2.5,0.5)
catch.prev$var1[catch.prev$catch.shed==0] = rnorm(sum(catch.prev$catch.shed==0),1,0.5)

beta.var1 = as.numeric(glm(catch.shed ~ as.numeric(scale(var1)), data = catch.prev, family = binomial)$coefficients[2])
...
set.seed(4444)
catch.prev$var2[catch.prev$catch.shed==1] = rnorm(sum(catch.prev$catch.shed==1),1,1)
catch.prev$var2[catch.prev$catch.shed==0] = rnorm(sum(catch.prev$catch.shed==0),0,1)

beta.var2 = as.numeric(glm(catch.shed ~ as.numeric(scale(var2)), data = catch.prev, family = binomial)$coefficients[2])
...
beta.var3 = as.numeric(glm(catch.shed ~ as.numeric(scale(var3)), data = catch.prev, family = binomial)$coefficients[2])
...
set.seed(123)
catch.prev$var4[catch.prev$catch.shed==1] = sample(c("A","B","C"), sum(catch.prev$catch.shed==1),replace = T, prob = c(1 + 0.3, 1, 1 - 0.3))
catch.prev$var4[catch.prev$catch.shed==0] = sample(c("A","B","C"), sum(catch.prev$catch.shed==0),replace = T, prob = c(1 - 0.3, 1, 1 + 0.3))


beta.var4 = as.numeric(glm(catch.shed ~ var4, data = catch.prev, family = binomial)$coefficients[c(2,3)])
...
set.seed(111)
plot1 = ggplot(data = catch.prev, aes(x = catch.times, y = catch.shed.false.neg)) +
       geom_line(data = sim.prev, aes(x = x, y = y.spline), size = 2, color = "grey30") +
       geom_jitter(data = catch.prev[catch.prev$catch.shed.false.neg==0 & catch.prev$catch.shed.false.neg.indicator==0,], color = brewer.pal(n = 11,"RdYlBu")[11], height = 0.075, width = 5, alpha = 1, shape = 21, size = 1.75) +
       geom_jitter(data = catch.prev[catch.prev$catch.shed.false.neg==0 & catch.prev$catch.shed.false.neg.indicator==1,], color = brewer.pal(n = 11,"RdYlBu")[1],fill = brewer.pal(n = 11,"RdYlBu")[1], height = 0.075, width = 5, alpha = 1, shape = 25, size = 2) +
       geom_jitter(data = catch.prev[catch.prev$catch.shed.false.neg==1,], color = brewer.pal(n = 11,"RdYlBu")[1], height = 0.075, width = 5, alpha = 1, shape = 21, size = 1.75) +
       scale_y_continuous(breaks = seq(0,1,0.1)) +
       scale_color_manual(values = col.2.groups[c(2,1)], name = "False\nnegative", labels = c("No","Yes")) +
       scale_fill_manual(values = c("white",col.2.groups[1]), name = "False\nnegative", labels = c("No","Yes")) +
       #scale_y_continuous(limits = c(0,1)) +
       ylab("Prevalence") +
       xlab("Time") +
       #ggtitle("Simulated individual sample infection status") +
       theme_bw() +
...
set.seed(111)
plot2 = ggplot(data = underroost.prev, aes(x = underroost.times, y = posneg)) +
       geom_line(data = sim.prev, aes(x = x, y = y.spline), size = 2, color = "grey30") +
       geom_jitter(data = underroost.prev[underroost.prev$pooled.ct==0,], color = brewer.pal(n = 11,"RdYlBu")[11], height = 0.075, width = 5, alpha = 1, shape = 21, fill = "white", size = 1.75) +
       geom_jitter(data = underroost.prev[underroost.prev$pooled.ct>0,], aes(color = pooled.ct), height = 0.075, width = 5, alpha = 1, shape = 21, fill = "white", size = 1.75) +
       scale_color_gradientn(colors = rev(brewer.pal(n = 11,"RdYlBu"))[7:11], name = "Ct") +
       scale_y_continuous(breaks = seq(0,1,0.1)) +
       ylab("Prevalence") +
       xlab("Time") +
       #ggtitle("Simulated pooled sample Ct values") +
       theme_bw() +
       theme(plot.title  =  element_text(size = 14),
             text = element_text(size = 18),
             axis.text = element_text(size = 18),
...
set.seed(111)
plot3 = ggplot(data = catch.prev, aes(x = catch.shed, y = var1, color = factor(catch.shed))) +
       geom_jitter(height = 0, width = 0.2, shape = 21, size = 1.75) +
       scale_color_manual(values = brewer.pal(n = 11,"RdYlBu")[c(11,1)], guide = "none") +
       scale_x_continuous(breaks = c(0,1), label = c("Neg","Pos")) +
       scale_y_continuous(breaks = (-1):4, limits = c(-1,3.5)) +
       ylab("Value") +
       xlab("") +
       #ggtitle("Simulated covariates") +
       theme_bw() +
       theme(plot.title  =  element_text(size = 14),
             text = element_text(size = 18),
             axis.text = element_text(size = 18),
...
set.seed(111)
plot4 = ggplot(data = catch.prev, aes(x = catch.shed, y = var2, color = factor(catch.shed))) +
       geom_jitter(height = 0, width = 0.2, shape = 21, size = 1.75) +
       scale_color_manual(values = brewer.pal(n = 11,"RdYlBu")[c(11,1)], guide = "none") +
       scale_x_continuous(breaks = c(0,1), label = c("Neg","Pos")) +
       scale_y_continuous(breaks = (-3):5, limits = c(-3,4.6)) +
       ylab("Value") +
       xlab("") +
       theme_bw() +
       theme(plot.title  =  element_text(size = 14),
...
set.seed(111)
plot5 = ggplot(data = catch.prev, aes(x = catch.shed, y = var3, color = factor(catch.shed))) +
       geom_jitter(height = 0, width = 0.2, shape = 21, size = 1.75) +
       scale_color_manual(values = brewer.pal(n = 11,"RdYlBu")[c(11,1)], guide = "none") +
       scale_x_continuous(breaks = c(0,1), label = c("Neg","Pos")) +
       scale_y_continuous(breaks = (-4):4, limits = c(-3.7,3.5)) +
       ylab("Value") +
       xlab("Status") +
       theme_bw() +
       theme(plot.title  =  element_text(size = 14),
...
plot.all = (plot1 + (plot3 / plot4 / plot5) + plot2) +
...
# ggsave(plot = plot.all,filename = "figures/Simulated data composite figure.png",width = 16, height = 7, dpi=600)
```
...
```{r}
# export simulated data for use in other analyses
# save(catch.prev, underroost.prev, sim.prev, total.time, prop.false.neg, beta.var1, beta.var2, beta.var4, time.int, n.samp.underroost, n.samp.catch, file = "underroost_and_catch_simulated_data.Rdata")
...
```{r}
#Ct probabilities
ct.prob.array = readRDS("ct.prob.39cutoff.array.observed.RDS")
```
...
stan.model.prev.Ct = 
       ...
       functions {
              ...
              vector dbin(int ntotal, real prob){
                     vector[ntotal+1] probdens;
                     for(i in 1:(ntotal+1)) {
                            probdens[i] = choose((ntotal), (i-1)) * prob^(i-1) * (1-prob)^(ntotal-(i-1));
                     }
                     return(probdens);
              }
              
              // probability mass function (lpmf)
              real ct_prob_lpmf(int ct, int nbat, real prev, vector ct_probs){
                     ...
                     vector[nbat+1] ctprob;       // vector for probabilities
                     ...
                     //}
                     ctprob = dbin(nbat, prev) .* ct_probs[1:(nbat+1)];
       ...
       data {
              ...
              int Ct_prob_Ntotal;
              int Ct_prob_Ct;
              real<lower=0> ct_array[Ct_prob_Npos,Ct_prob_Ntotal,Ct_prob_Ct];        // array P(Npos|Ct,Ntotal)
       ...
       transformed data {
              ...
              vector<lower=0>[Ct_prob_Npos] ct_array_probs[N_ur];
              
              for(i in 1:N_ur){
                     ct_array_probs[i] = to_vector(ct_array[,bat_count[i],ct_index[i]]);
              }
       }
       ...
       model {
              ...
              betas ~ normal(0,5);        // slightly informative regularizing prior for regression coefficients individual shedding
              ...
              //y_catch ~ bernoulli_logit(y_catch_bern_mu);
...
catch.vars.matrix = matrix(data = c(as.numeric(scale(catch.prev$var1))), byrow = F, ncol=1)
...
if(run.model.fit.again == T) {
       ...
       saveRDS(model.fit,"stan.model.prev.Ct.fit.20220913.RDS")
} else {
       model.fit = readRDS("stan.model.prev.Ct.fit.20220913.RDS")
}
...
rownames(sum.fit)[which(sum.fit$Rhat>1.01)]
...
ggplot() + 
       geom_density_ridges_gradient(data = plot.df, aes(x = x, y = 0, fill = stat(quantile)), quantile_lines = TRUE, quantile_fun = hdi, vline_linetype = 2) +
       geom_point(data = data.frame(x = beta.var1, y = 0), aes(x=x, y=y), col = "black", size = 4) +
       scale_fill_manual(values = c("transparent", col.no.groups, "transparent"), guide = "none") +
       ylab("Posterior density") +
...
# posbat.pred.df[which(posbat.pred.df$n.bats>6),]
# underroost.prev[which(underroost.prev$n.bats.total>6),]
```
...
catch.vars.matrix = matrix(data = c(as.numeric(scale(catch.prev$var3))), byrow = F, ncol=1)
...
True coefficient var 3 = `r beta.var3`.           
...
ggplot() + 
       geom_density_ridges_gradient(data = plot.df, aes(x = x, y = 0, fill = stat(quantile)), quantile_lines = TRUE, quantile_fun = hdi, vline_linetype = 2) +
       geom_point(data = data.frame(x = beta.var3, y = 0), aes(x=x, y=y), col = "black", size = 4) +
       scale_fill_manual(values = c("transparent", col.no.groups, "transparent"), guide = "none") +
       ylab("Posterior density") +
       xlab("Effect estimate") +
       ggtitle("") +
