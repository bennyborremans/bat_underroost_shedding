...
stan.model.prev.Ct = 
       ...
       functions {
              ...
              real ct_prob_lpmf(int ct, int nbat, real prev, real[] ct_probs){
                     ...
                     for(n in 1:(nbat+1)) {
                            ctprob[n] = fabs(ct_probs[n] - dbin(n-1, nbat, prev)) ;
                     }
                     lprob = log((2 - sum(ctprob))/2);   
       ...
       model {
              phi_P_ur ~ gamma(2,1);       // larger values = lower variance, less heterogeneity in the outcome data
              ...
              eta ~ normal(0,2);
              mu ~ normal(0,2);
              //sigma_trunc ~ lognormal(log(0.25),log(1.3));   
              ...
              for(j in 1:N_ur) {
                     ...
                     // P_ur[j] ~ normal(Phi(theta_probit[day_idx_ur[j]]), sigma_trunc) T[trunc_lower, trunc_upper];      
       ...
       generated quantities {
              ...
              for(i in 1:N_catch){
                     p_shed[i] = inv_logit(logit(Phi(theta_probit[day_idx_catch[i]])) + dot_product(betas, vars_catch[i]));
              }
...
# situation: Ct = 30, ntotal = 3
# is probability calculation correct?

ntotal = 4
ctprob = ct.prob.array[1:(ntotal+1),ntotal,11]
prevs = seq(0,1,0.1)
lprobs.mult.sum = numeric(length(prevs))
lprobs.lsum.sum = numeric(length(prevs))
lprobs.dist.sum = numeric(length(prevs))
probs.dist = numeric(length(prevs))
for(i in 1:length(prevs)){
       cur.dbinom = dbinom(0:ntotal,size = ntotal, prob = prevs[i])
       
       cur.probs.mult = ctprob * cur.dbinom
       lprobs.mult.sum[i] = log(sum(cur.probs.mult[1:4]))
       
       cur.probs.lsum = log(ctprob) + log(cur.dbinom)
       cur.probs.lsum[cur.probs.lsum==-Inf] = 0
       lprobs.lsum.sum[i] = sum(cur.probs.lsum[1:4])
       
       cur.probs.dist = (2-sum(abs(cur.dbinom - ctprob)))/2
       probs.dist[i] = cur.probs.dist
       lprobs.dist.sum[i] = log(cur.probs.dist)
}
lprobs.lsum.sum[lprobs.lsum.sum==0] = -99999
prevs[which.max(lprobs.mult.sum)]
prevs[which.max(lprobs.lsum.sum)]
prevs[which.max(lprobs.dist.sum)]
probs.dist

sum(abs(ctprob-dbinom(0:ntotal,size = ntotal, prob = 0.6)))
sum(abs(ctprob-dbinom(0:ntotal,size = ntotal, prob = 0.5)))
sum(abs(ctprob-dbinom(0:ntotal,size = ntotal, prob = 0.7)))



ctprob

cur.dbinom = dbinom(0:ntotal,size = ntotal, prob = 0.1)
cur.dbinom
ctprob * cur.dbinom
sum(cur.dbinom[2:4])   # 0.657
sum(ctprob * cur.dbinom)   # 0.1893203
sum(abs(cur.dbinom - ctprob))  # 1.08361
(2-sum(abs(cur.dbinom - ctprob)))/2  # 0.270195


cur.dbinom = dbinom(0:ntotal,size = ntotal, prob = 0.6)
cur.dbinom
ctprob * cur.dbinom
sum(cur.dbinom[2:4])  # 0.936
sum(ctprob * cur.dbinom)   # 0.3161729
sum(abs(cur.dbinom - ctprob))  # 0.31757
(2-sum(abs(cur.dbinom - ctprob)))/2  # 0.841215

cur.dbinom = dbinom(0:ntotal,size = ntotal, prob = 0.9)
cur.dbinom
ctprob * cur.dbinom
sum(cur.dbinom[2:4])  # 0.999
sum(ctprob * cur.dbinom)   # 0.3728352
sum(abs(cur.dbinom - ctprob)) # 0.71043
(2-sum(abs(cur.dbinom - ctprob)))/2  # 0.644785

cur.dbinom = dbinom(0:ntotal,size = ntotal, prob = 1)
cur.dbinom
ctprob * cur.dbinom
sum(cur.dbinom[2:4])  # 1
sum(ctprob * cur.dbinom)   # 0.37479
sum(abs(cur.dbinom - ctprob))  # 1.25043
(2-sum(abs(cur.dbinom - ctprob)))/2  # 0.374785
...
sum(abs(ctprob[1:4] - dbinom(c(0,1,2,3),size = 3, prob = 0.1)))
sum(abs(ctprob[1:4] - dbinom(c(0,1,2,3),size = 3, prob = 0.4)))
sum(abs(ctprob[1:4] - dbinom(c(0,1,2,3),size = 3, prob = 0.6)))
sum(abs(ctprob[1:4] - dbinom(c(0,1,2,3),size = 3, prob = 0.8)))
sum(abs(ctprob[1:4] - dbinom(c(0,1,2,3),size = 3, prob = 1)))


(4 - sum(abs(ctprob[1:4] - dbinom(c(0,1,2,3),size = 3, prob = 0.1))))/4
(4 - sum(abs(ctprob[1:4] - dbinom(c(0,1,2,3),size = 3, prob = 0.4))))/4
(4 - sum(abs(ctprob[1:4] - dbinom(c(0,1,2,3),size = 3, prob = 0.6))))/4
(4 - sum(abs(ctprob[1:4] - dbinom(c(0,1,2,3),size = 3, prob = 0.8))))/4
(4 - sum(abs(ctprob[1:4] - dbinom(c(0,1,2,3),size = 3, prob = 1))))/4

cur.prob = numeric(length(prevs))
for(i in 1:length(prevs)){
       cur.prob[i] = ((ntotal+1) - sum(abs(ctprob[1:4] - dbinom(c(0,1,2,3),size = ntotal, prob = prevs[i]))))
}
prevs[which.max(cur.prob)]


beta(Phi(theta_probit[day_idx_ur[j]]) * phi_P_ur, (1 - Phi(theta_probit[day_idx_ur[j]])) * phi_P_ur);    

prop = 0.1
phi = 20
hist(rbeta(10000,prop * phi, (1-prop) * phi));mean(rbeta(10000,prop * phi, (1-prop) * phi))

prop = 0.1
sigma = 0.4
hist(rnorm(10000,prop,sigma), xlim = c(0,1));mean(rnorm(10000,prop,sigma), xlim = c(0,1))
...
if(run.model.fit.again == T) {
       model.fit = stan(model_code = stan.model.prev.Ct,
                        ...
                        iter = 5000,      
                        cores = 5) 
       #saveRDS(model.fit,"stan.model.prev.Ct.fit.20220805.RDS")
} else {
       #model.fit = readRDS("stan.model.prev.Ct.fit.20220805.RDS")
}
