...
In order to get a better sense of the observation process resulting in pooled samples with certain Ct values,   
a number of aspects are investigated and simulated.   
...
plot1 = ggplot(data = ct2.prob, aes(x = ct, y = Freq)) +
       geom_bar(stat = "identity", fill = col.no.groups, col = "black") +
       xlab("Ct") +
       ylab("Frequency") +
       scale_x_continuous(breaks = 21:39) +
...
# ggsave(plot = plot1,filename = "Ct value individual histogram.png",width = 6, height = 4, dpi=600)
...
plot1 = ggplot(data = ct.fit.prob, aes(x = ct, y = prob)) +
...
# ggsave(plot = plot1,filename = "Ct value individual fitted probabilities.png",width = 6, height = 4, dpi=600)


# save ct.fit.prob
# saveRDS(ct.fit.prob, "ct.fit.prob.RDS")
```
...
Given a few assumptions, we can now estimate shedding prevalence using the probability of seeing a certain Ct value in a pooled sample, 
given the number of contributing bats and given a distribution describing the individual probability of shedding at a certain Ct value.
...
This means that we have to ignore this effect of evaporation, and hence assume that there is none.   

\vspace{15pt}

The assumption of known number of contributing bats is less strict.    
Although the current (2022-07-26) version of the model assumes that numbers are known,  
it is possible to allow uncertainty in this number (by treating the number of bats contributing to a sample as a parameter to be estimated, with a prior distribution informed by actual information, which can be something like "3-5 bats").    
...
Priors:     
$\theta_{true} \sim Beta(1,1)$
$\phi \sim Gamma(2,1)$  (variation parameter, small values allow more variation, which is necessary here because the small sample sizes (N bats contributing to a pooled sample) need a lower resolution.    


$i$ = pooled sample    
$Ct$ = Ct value of a pooled sample     
$N$ = number of bats contributing to a pooled sample    
$\theta_i$ = "prevalence" in a pooled sample (proportion of positive bats contributing to the pooled sample), follows a distribution with mean $\theta_{true}$     
$\theta_{true}$ = true shedding prevalence in the population     
...
range.Npos = 0:15
range.Ct = c(0,21:39)
ct.prob.array = array(data = 0, dim = c(length(range.Npos), length(range.Ntotal), length(range.Ct)), dimnames = list(Npos = range.Npos,
                                                                                                                     Ntotal = range.Ntotal,
                                                                                                                     Ct = range.Ct))
# values of 0 in ct.prob.array were NA at first because they don't exist, but stan complains when there are NA's in arrays so need a value
...
if(run.again == T) {
       ...
       for(i in 1:length(range.Ct)){
              ...
              for(j in 1:length(range.Ntotal)){
                     
                     cur.n.total = range.Ntotal[j]
                     
                     for(k in 2:(cur.n.total+1)){
                            
                            
                            cur.n.pos = range.Npos[k]
                            
                            # read in sums of genome copies and probabilities
                            # necessary to import every time so that the memory can be cleared (otherwise vector memory exhausted at large npos),
                            # even though this slows things down a bit
                            cur.sums.perms = readRDS(paste0("genome.copy.sum.list.",cur.n.pos,".RDS"))
                            cur.sums.probs = readRDS(paste0("probs.sum.list.",cur.n.pos,".RDS"))
                            
                            # calculate mean for each combination, including the negatives (i.e. divide by total number, not just by number of positives)
                            comb.means.gencop = cur.sums.perms/cur.n.total
                            rm(cur.sums.perms);gc()
                            ...
                            rm(comb.means.ct.round);gc()
                            if(length(comb.possible.idx) == 0) cur.prob = 0
                            if(length(comb.possible.idx) > 0) {
                                   
                                   # sum probabilities of selected combinations, normalized
                                   ##########################
                                   ####   !! this step can be done separately, so computation can be sped up a lot.
                                   ####      can be very useful when dealing with e.g. seasonally varying Ct distributions.
                                   ####      I think all that needs to be done is store every comb.possible.idx, then do the next step and store it in the right place in the array
                                   ##########################
                                   cur.prob = sum(cur.sums.probs[comb.possible.idx])/sum(cur.sums.probs)  
                                   rm(comb.possible.idx, cur.sums.probs);gc()
                                   
                            }
                            ct.prob.array[k,j,i] = cur.prob
                            print(paste0("Ct = ",cur.ct,", Npos = ", cur.n.pos, ", Ntotal = ",cur.n.total,", Prob = ",cur.prob))
                            rm(cur.prob);gc()
                     }
                     
                     if(cur.ct == 0) {
                            ct.prob.array[1,j,i] = 1
                     }
                     
                     # normalization:   (old, not doing this because it messes up the dot product with binomial densities
                     # if(cur.ct == 0) {
                     #        ct.prob.array[1,j,i] = 1 - sum(ct.prob.array[,j,i])
                     # } else {
                     #        # normalize probabilities (sum to 1 per Ntotal)
                     #        ct.prob.array[,j,i] = round(ct.prob.array[,j,i]/sum(ct.prob.array[,j,i],na.rm=T),5)
                     # }
                     print(ct.prob.array[,j,i])
              }
       }
...
stan.model.ur.prev.Ct = 
       ...
       model {
              phi ~ gamma(2,1);       // larger values = lower variance, less heterogeneity in the outcome data
              prev_ur ~ beta(1,1);
...
```{r}
# simulate some data, see higher
sim.prev = 0.27
sim.N = 40
set.seed(222)
simdat = ur.sim.fun(prev = sim.prev, n = sim.N)

# round Ct values, same way as that used to calculate probabilities
sim.Ct = ceiling(round(simdat[[1]][,1],1))

# change 40 to 0
sim.Ct[sim.Ct==40] = 0

sim.bat.count = simdat[[1]][,2]

# index for ct_array
ct.index = numeric(sim.N)
ct.index[sim.Ct==0] = 1
ct.index[sim.Ct>0] = sim.Ct[sim.Ct>0] - 19

```

The simulated Ct values: `r sim.Ct`.    
Number of bats per pooled sample: `r sim.bat.count`.    



```{r}

model.fit = stan(model_code = stan.model.ur.prev.Ct,
                 data = list(N = sim.N,
                             Ct = sim.Ct,
                             ct_index = ct.index,
                             bat_count = sim.bat.count,
...
```{r fig.height = 9, fig.width = 5, out.width = "30%"}
# means = 
means = summary(model.fit)$summary[,1]


fit.posterior = as.array(model.fit)

mcmc_areas(fit.posterior[,,1],
           prob = 0.8,
           prob_outer = 0.95)
```


```{r fig.height = 10, fig.width = 12, out.width = "90%"}
mcmc_trace(fit.posterior[,,1:10])
```


```{r fig.height = 10, fig.width = 7, out.width = "40%"}
# compare simulated samples and individual estimated prevalence
simdat[2:10]
sim.Ct[1:9]
mcmc_areas(fit.posterior[,,2:10],
           prob = 0.8,
           prob_outer = 0.95)
```
...
simdat[11:20]
sim.Ct[10:19]
mcmc_areas(fit.posterior[,,11:20],
           prob = 0.8,
           prob_outer = 0.95)
...
```{r fig.height = 10, fig.width = 7, out.width = "30%"}
means = summary(model.fit)$summary[,1]
means

fit.posterior = as.array(model.fit)

mcmc_areas(fit.posterior[,,1:5],
           prob = 0.8,
           prob_outer = 0.95)
```
